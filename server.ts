...
// Initialize the GoogleGenAI instance on the server-side only
...
if (apiKey) {
  ...
  console.warn("WARNING: GEMINI_API_KEY environment variable is not set. AI Consultant will operate in mock/fallback mode.");
}
  ...
  app.post("/api/consult", async (req, res) => {
    try {
      ...
      if (!ai) {
        ...
        return res.json({
          text: `[Chế độ dự phòng] Cửa sổ tư vấn lương hoạt động bình thường, tuy nhiên bạn cần cấu hình khóa API trong mục Secrets để trò chuyện thực tế với AI. \n\nDưới đây là thông tin chung về Nghị định 182/2026/NĐ-CP:\n- Mức lương cơ sở mới đề xuất là 2.340.000 VNĐ.\n- Các phụ cấp chính bao gồm: Phụ cấp ưu đãi nghề (ví dụ 35% cho giáo viên/y tế cơ sở), Phụ cấp công vụ (25% cho công chức hành chính), và Phụ cấp thâm niên (giáo sư, giáo viên quân đội, công an sau 5 năm công tác).`
        });
      }
...
Câu trả lời của bạn nên định dạng đẹp bằng Markdown để dễ đọc, trình bày rõ ràng, thân thiện với các công chức, giáo viên, bác sĩ.`;
      ...
      res.status(500).json({ error: error.message || "Lỗi xử lý tư vấn lương." });
    }
  });
